...
async function initDashboard() {
    ...
    if (!session) {
        window.location.href = 'https://killersvip.com/login.html';
        return;
    }
    ...
    if (userError) {
        ...
        if (!dbUser || !dbUser.promo_code_used) {
            window.location.href = 'https://killersvip.com/register.html';
            return;
        }
        currentUser = dbUser;
...
function generateReferralLink(user) {
    ...
    const refUrl = `https://killersvip.com/register.html?ref=${user.display_id}`;
...
function selectPkg(name, defaultPrice) {
    ...
    window.location.href = `https://killersvip.com/payment.html?pkg=${encodeURIComponent(name)}`;
}
